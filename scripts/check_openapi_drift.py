...
FIX = (
    "FIX: Do not hand-edit openapi.json. From bitlync-core run "
...
def main() -> int:
    ...
    # Stronger check when a core-read token is available (private core).
    ...
    if live is not None:
        ...
        # Tip version must match pin version when we can read version.py
        ...
        print(f"ok: openapi.json matches lock and live core pin {pin_path} ({version})")
    else:
        print(
            f"ok: openapi.json matches {LOCK.name} ({version}, sha256={digest[:12]}…). "
